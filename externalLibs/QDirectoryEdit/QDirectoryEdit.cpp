...
{
    QAbstractItemView* comp = dynamic_cast< QAbstractItemView* >( p_obj );

    if( !comp->currentIndex().isValid() )
    {
        // if nothing is selected, push down then enter
        QKeyEvent key1( QEvent::KeyPress, Qt::Key_PageDown, Qt::NoModifier );
        QApplication::sendEvent( p_obj, &key1 );

        QKeyEvent key2 = QKeyEvent( QEvent::KeyPress, Qt::Key_Enter, Qt::NoModifier );
        QApplication::sendEvent( p_obj, &key2 );
    }
    else
    {
